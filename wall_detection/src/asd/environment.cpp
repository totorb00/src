...
int read_uwb_record_all(std::string uwb_file_path, std::string uwb_file_name, std::vector<std::vector<std::string>> &uwb_data, bool print_yes)
{
    ...
    if(!uwb_rec_file_obj){
        ...
        Eigen::Matrix2f kf_uwb_Q = Eigen::Matrix2f::Identity();
        kf_uwb_Q(0,0)=0.005;
        kf_uwb_Q(1,1)=0.01;
        ...
        std::vector<std::string> uwb_first_row = uwb_data[0];
        Eigen::Vector2f kf_uwb_last_fine_X (std::stof(uwb_first_row [7]), std::stof(uwb_first_row [8]));

        for(std::vector<std::string>& row:uwb_data)
        {
            kf_uwb_X_meas << std::stof(row[7]), std::stof(row[8]);
            // ++ ---------------
            if( (kf_uwb_last_fine_X-kf_uwb_X_meas).norm() > 7 )
                kf_uwb_X_meas = kf_uwb_last_fine_X;
            // ++ ---------------
            // ++ ------------
...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer, ProcessPointClouds<pcl::PointXYZI>* pointProcessorI, pcl::PointCloud<pcl::PointXYZI>::Ptr inputCloud, Eigen::Vector3d &uwb_loc_rect, std::vector<Eigen::Vector3f> &objs_in_last_scene)
{
  // ----------------------------------------------------
...
int main (int argc, char** argv)
{
    ...
    if(export_objs_loc){
        ...
        objs_loc_File = std::ofstream("/home/reamond/Documents/objs_location_2023-10-04.txt");
        if(!objs_loc_File){
...
// return -1;
    ...
    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    // pcl::PointXYZ uwb_ploc_rect( 0, 0, 0 );
    Eigen::Vector3d uwb_loc_rect(0,0,0);
    std::vector<Eigen::Vector3f> objs_in_last_scene;
    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    while (!viewer->wasStopped ())
    {
        ...
        if(with_uwb){
            ...
            if (get_uwb_data==1){
                ...
                // uwb_ploc_rect = pcl::PointXYZ(uwb_loc_rect(0), uwb_loc_rect(1), uwb_loc_rect(2));
